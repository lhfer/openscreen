...
function PlaybackCursor({
	...
	return (
		...
		>
			<div
				className="absolute top-0 bottom-0 w-[2px] bg-[#4A90E2] shadow-[0_0_10px_rgba(52,178,123,0.5)] cursor-ew-resize pointer-events-auto hover:shadow-[0_0_15px_rgba(52,178,123,0.7)] transition-shadow"
				style={{
			...
			>
				...
				>
					<div className="w-3 h-3 mx-auto mt-[2px] bg-[#4A90E2] rotate-45 rounded-sm shadow-lg border border-white/20" />
				</div>
				{isDragging && (
						...
						<div className="flex flex-col items-center pb-1">
							...
							<span
								className={cn(
									...
									marker.time === currentTimeMs ? "text-[#4A90E2]" : "text-slate-500",
								)}
							>
								{marker.label}
							</span>
...
export default function TimelineEditor({
	...
	const [scrollLabels, setScrollLabels] = useState({
		pan: t("hints.scrollToPan"),
		zoom: t("hints.scrollToZoom"),
	});
	...
	useEffect(() => {
		formatShortcut(["mod", "Scroll"]).then((zoom) => {
			setScrollLabels({ pan: t("hints.scrollToPan"), zoom });
		});
	}, []);
			...
			<div className="flex items-center gap-2 px-4 py-2 border-b border-white/5 bg-[#09090b]">
				<div className="flex items-center gap-1">
					<Button
						...
						variant="ghost"
						size="icon"
						className="h-7 w-7 text-slate-400 hover:text-[#4A90E2] hover:bg-[#4A90E2]/10 transition-all"
						...
						<DropdownMenuContent align="end" className="bg-[#1a1a1a] border-white/10">
							{ASPECT_RATIOS.map((ratio) => (
								...
								>
									...
									{aspectRatio === ratio && <Check className="w-3 h-3 text-[#4A90E2]" />}
								</DropdownMenuItem>
							))}
						</DropdownMenuContent>
					</DropdownMenu>
				...
				<div className="flex items-center gap-4 text-[10px] text-slate-500 font-medium">
					<span className="flex items-center gap-1.5">
						<kbd className="px-1.5 py-0.5 bg-white/5 border border-white/10 rounded text-[#4A90E2] font-sans">
							{scrollLabels.pan}
						</kbd>
